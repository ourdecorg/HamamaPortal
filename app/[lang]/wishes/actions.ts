...
export async function analyzeWish(_prev: WishState, formData: FormData): Promise<WishState> {
  ...
  const parsed = makeWishSchema(m.wishTooShort).safeParse(raw);
  if (!parsed.success) {
    return { status: "error", error: parsed.error.issues[0]?.message ?? m.formInvalid, values: raw };
  }
  const v = parsed.data;
  const result = await discover({ query: v.wish, projects: await getProjects(), context: discoveryContext(v), locale });
 ...
 */
export async function saveWish(values: WishValues, localeArg: string): Promise<SaveWishResult> {
  ...
  const parsed = makeWishSchema(m.wishTooShort).safeParse(values);
  if (!parsed.success) return { status: "error", error: parsed.error.issues[0]?.message ?? m.wishInvalid };
  const v = parsed.data;
  const result = await discover({ query: v.wish, projects: await getProjects(), context: discoveryContext(v), locale });
  ...
  if (error || !data) {
    ...
    return { status: "error", error: m.wishSaveFailed };
  }
