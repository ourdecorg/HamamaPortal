...
export async function generateMetadata(): Promise<Metadata> {
  return { title: (await getMessages()).meta.login.title, robots: { index: false } };
}
...
export default async function LoginPage({
  ...
  const m = (await getMessages()).login;
  const next = safeNext(typeof sp.next === "string" ? sp.next : undefined, `/${locale}`);
  const error = typeof sp.error === "string" && sp.error in m.errors ? m.errors[sp.error as keyof typeof m.errors] : undefined;
