...
/**
 * "I want to pursue this connection".
 *
 ...
 */
export async function advanceConnection(connectionId: string, locale: string): Promise<AdvanceResult> {
  const m = actionMessages(locale).actions;
  if (!isSupabaseConfigured()) return { status: "error", error: m.saveUnconfigured };
  const user = await getCurrentUser();
  if (!user) return { status: "auth_required" };

  const id = connectionIdSchema.safeParse(connectionId);
  if (!id.success) return { status: "error", error: m.invalidConnection };

  // The rationale saved with the opportunity is written in the language of the person who advanced it.
  const connection = (await getConnections(actionLocale(locale))).find((c) => c.id === id.data);
  if (!connection) return { status: "error", error: m.connectionGone };
  ...
  if (!existing) {
    ...
    if (error?.code === "23505") {
      ...
      return { status: "error", error: m.connectionSaveFailed };
    } else {
  ...
  if (!existing) return { status: "error", error: m.connectionSaveFailed };
...
async function changeStatus(
  ...
  if (!isSupabaseConfigured() || !(await getCurrentUser())) return { status: "error", error: m.signInToContinue };
  const parsed = uuidSchema.safeParse(id);
  if (!parsed.success) return { status: "error", error: m.invalidConnection };
  ...
  const { data, error } = await supabase
    ...
    .select("id, status")
    .maybeSingle();
  if (error || !data) return { status: "error", error: m.connectionUpdateFailed };
...
/**
 * "Ask for an introduction": records that I intend to ask for an introduction. It does NOT contact anyone —
 * the person copies the drafted message and sends it themselves.
 */
export async function requestIntroduction(opportunityId: string, locale: string): Promise<StatusChangeResult> {
  return changeStatus(opportunityId, "intro_requested", ["interested", "intro_requested"], actionMessages(locale).actions);
}

/** "Not pursuing for now": close one of my own opportunities. */
export async function closeOpportunity(opportunityId: string): Promise<void> {
  await changeStatus(opportunityId, "closed", ["interested", "intro_requested"], actionMessages(undefined).actions);
}
